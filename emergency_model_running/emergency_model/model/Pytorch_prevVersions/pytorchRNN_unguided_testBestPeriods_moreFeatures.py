...
def getData(x_length, y_length, periods, sampling_method):
	...
	seq_inOr_tra  = train_in.transpose(0,1)
	seq_outOr_tra = train_out.transpose(0,1)


	val_size = len(val_in)
	print(val_size)
	seq_inOr_val = val_in.transpose(0,1)#torch.Size([15, 1334, 1])
	seq_outOr_val = val_out.transpose(0,1)

	test_size = len(test_in)
	seq_inOr_tst = test_in.transpose(0,1)#torch.Size([15, 1334, 1])
	seq_outOr_tst = test_out.transpose(0,1)
...
logFolder = os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResultsSubType.csv")
dfResults = pd.read_csv(logFolder)
...
for index, row in dfResults.iterrows():
	...
	x_dim = int(row["inputDim"])
	y_dim = int(row["outputDim"])
	...
	print(seq_inOr_tst.shape, seq_outOr_tst.shape )
...
dfResults.to_csv(os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResultsSubType.csv"), index=False)
