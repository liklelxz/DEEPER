...
class LSTM(nn.Module):
	...
	def forward(self, input_seq, output_seq, batch_size,hidden_cell, pred_length):
		...
		for step in range(pred_length):
			lstm_outDec, hidden_cellDec = self.lstmEnc(dec_input.view(1,batch_size, -1), hidden_cellDec)
...
type_rnn = args.type_rnn + "_unguided_same"
...
with open(os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResults.csv"),'a') as f:
    ...
    csv_reader.writerow([type_rnn, args.epochs,x_dim,str(args.x_length),str(args.y_length),args.learning_rate, \
    					args.hidden_layer, args.type_training, sizeTrain, args.mini_batch, args.type_actifun, round(loss_train,3), round(loss_tst,3), round(err_avg,0), round(diffMaxMin,2), round(stdPreds,4),folder,"",file_name ])
