...
X_train_data, Y_train_data, X_test_data, Y_test_data = getData(args.data_path,x_length,y_length,0.75, x_dim,args.inputCols, 0)
...
'''
if typeScale=='mm':
    YreTrainScale = scaler.inverse_transform()
...
def train(epoch, model):
	...
	#mseLoss = nn.MSELoss()
	#mse_loss = mseLoss(predictions[-y_length:,:,:], dataY.transpose(0, 1))
	...
	'''
	rmse1 = RMSE(Y_train_data[:,:, 0], predictions[:,:,0])
	rmse2 = RMSE(Y_train_data[:,:, 1], predictions[:,:,1])

	rmse1_scaled = np.round(np.mean(rmse1), 2)
	rmse2_scaled = np.round(np.mean(rmse2), 2)
	'''
...
def test(epoch, model):
	...
	rmse1 = RMSE(Y_test_data[:,:, 0], predictions[:,:,0])
	...
	Energy = np.concatenate([ numPred1[:,step,None], Y_test_data[:,step, 0,None]],axis=1 )
...
weights_o = model.state_dict()['fc.weight'].detach().numpy()
