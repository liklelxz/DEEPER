...
from preprocessing.preprocessorPeriods_feature import *
...
#parser.add_argument('--columns', type=str, default='TS', metavar='FS',help='Add the following columns as features')
...
#Setting parameters
columns = ['Response Time','Subtype']# args.columns
...
type_rnn = args.type_rnn + "_unguided_feaures"
...
sets, stats, indexRef = getPreProcessedData(incident_group[:3], x_length, y_length, periods, sampling_method, columns)
...
'''
qTra50 = scaler.transform(np.array([qTra50]).reshape(-1,1))
qTra90 = scaler.transform(np.array([qTra90]).reshape(-1,1))
qVal50 = scaler.transform(np.array([qVal50]).reshape(-1,1))
qVal90 = scaler.transform(np.array([qVal90]).reshape(-1,1))
qTest50= scaler.transform(np.array([qTest50]).reshape(-1,1))
qTest90= scaler.transform(np.array([qTest90]).reshape(-1,1))
'''
...
def testing(seq_inOr, seq_outOr, size):
	...
	for i in range(size):
		...
		with torch.no_grad():
			...
			preds=model(seq_inOr[:,i,None,:], seq_outOr[:,i,None,:] , 1,hidden, y_length)
	...
	## Getting inverse transformation of quantile that was under scaling
	ratioPeaks = NUMPEAK(real_test_save,pred_test_save, scaler.inverse_transform(np.array([qTra50]).reshape(-1,1)))
...
thresholdStd = float(qTra50)#torch.Tensor([float(qVal50)])
bestEpoch = 0
...
for e in range(epochs):
	...
	if e==0 or e%(epochs/print_every) == 0 or e==epochs-1:
		...
		#print(model_parameters[0][1].grad.data.norm(2).item(),4)
		#print(model.parameters()[0][1].grad.data.norm(2).item(),4)
		...
		#modelDict = model.state_dict()#['model_state_dict']
		#print(modelDict['lstmEnc.weight_hh_l0'][0][:5])
		...
		#Plot pred vs real -  TESTING
		step_plot = 0
		plt.plot(np.concatenate((pred_test[step_plot],seq_outOr_tst[step_plot]),axis =1))
		plt.savefig("{}/PredvsRealTest_{}".format(save_path,e), bbox_inches='tight')#self.savedFolder+'/'+ch.name+str(numfig)
		plt.clf()
...
# To decide if we prefer to choose the last values with a possible low RMSE on the test but very low std not passing the thrshold
#if bestLossVal == float('Inf')
#	bestEpoch=epochs-1
...
loss_tst = loss_test.item()#to bring from graph network space
...
#Plot boxplot of losses
dfBox = pd.DataFrame(loss_box_test)
dfBox.boxplot()
...
dfGroup['Response Time'].iloc[idxStepX] = seq_inOr_tst[step_plot,:,0].detach().numpy().squeeze()
dfGroup['Response Time'].iloc[idxStepY] = seq_outOr_tst[step_plot,:,0].detach().numpy().squeeze()
...
plt.savefig("{}/PredvsReal_TestRealSort".format(save_path), bbox_inches='tight')#self.savedFolder+'/'+ch.name+str(numfig)
plt.clf()
...
#Final log
print(f'Final: {e:3} Train loss: {loss_train:10.4f} Test loss: {loss_tst:10.4f} {err_test:10.4f} Diff Max-Min :{diffMaxMin:10.4f} Std: {stdPreds:2.4f}')
...
#folder, Comments
with open(os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResultsSubType.csv"),'a', encoding='utf-7') as f:
    csv_reader = csv.writer(f, delimiter=',')
    csv_reader.writerow([type_rnn, args.epochs,x_dim,y_dim,str(args.x_length),str(args.y_length),args.learning_rate, args.hidden_layer, args.type_training, \
    						sizeTrain, args.mini_batch, args.type_actifun, str(periods), sampling_method, \
    						round(loss_train,3), round(loss_tst,3), round(err_test,0), round(diffMaxMin,2), round(stdPreds,4), round(numPeaksTest,4),  \
    						round(err_val,0), round(stdPredsTotalVal,4), round(numPeaksVal,4), \
    						bestEpoch, bestLosss, bestStd, round(bestPeaks,4) ,\
    						"","","","","","","","", folder,"",file_name ])
