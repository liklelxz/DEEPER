...
#print(train_normalized.max(), train_normalized.min(), valid_normalized.max(), valid_normalized.min(), test_normalized.max(), test_normalized.min())


print("Q-0.5: ",np.quantile(train_normalized,0.5), np.quantile(valid_normalized,0.5), np.quantile(test_normalized,0.5))
print("Q-0.9: ",np.quantile(train_normalized,0.9), np.quantile(valid_normalized,0.9), np.quantile(test_normalized,0.9))
...
def create_inout_sequences(input_data, xw, yw):
	...
	for i in range(L-xw-yw):
		train_seq = input_data[i:i+xw+yw-1] # +yw-1 to predict just next step, instead yw ahead
...
def training(seq_inOr, seq_outOr):
	...
	for i in range(num_batches):
		...
		y_pred, _ = model(seq_in, size, hidden_cell, y_length)
...
dfLog = pd.DataFrame(columns=['lossTrain','lossVal','standarDeviation'])
...
for e in range(epochs):
	...
	if e%(epochs/20) == 1 or (e==epochs-1):
		...
		loss_val, pred_val, loss_val_batch , loss_box, err = testing(seq_inOr_val, seq_outOr_val, val_size)
		loss_val = loss_val.item()
		diffVal = np.max(pred_val) -  np.min(pred_val)
		stdPreds = np.std(pred_val)
		print(f'epoch: {e:6} Train loss: {loss_train:10.6f} Validation loss-RMSE: {loss_val:10.6f} {err:10.6f} Diff Max-Min Pred: {diffVal:5.4f} Std: {stdPreds:2.2f}')
...
print(f'Final: {e:3} Train loss: {loss_train:10.4f} Test loss: {loss_tst:10.4f} {err_avg:10.4f} Diff Max-Min :{diffMaxMin:10.4f} Std: {stdPreds:2.2f}')
...
type_rnn = args.type_rnn + "_guided"

#Coppy and paste for the HEADER
#modelType, epochs,inputDim, xLenght, yLenght, learningRate, unitsHiddenLayer, typeTraining, trainingSize,miniBatchSize, typeAF, lossTrain, lossTest, diffMaxMin, stdDev, folder, Comments
with open(os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResults.csv"),'a') as f:
    ...
    csv_reader.writerow([type_rnn, args.epochs,x_dim,args.x_length,args.y_length,args.learning_rate, args.hidden_layer, args.type_training, \
    					sizeTrain, args.mini_batch, args.type_actifun, round(loss_train,3), round(loss_tst,3), round(err_avg,0), round(diffMaxMin,2),round(stdPreds,2),folder,"",file_name])
