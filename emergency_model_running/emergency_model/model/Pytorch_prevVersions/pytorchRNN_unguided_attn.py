...
type_rnn = args.type_rnn + "_unguided_attn"
...
if args.type_rnn == 'rnn':
	model = RNNAttn(x_dim, y_dim, hidden_units, args.type_actifun)
else:#lstm
	model = LSTMAttn(x_dim, y_dim, hidden_units, args.type_actifun)
...
dfLog.to_csv(os.path.join(save_path+'/LogAttn.csv'))

dfLogPerIter.index.name = 'Epoch'
dfLogPerIter.to_csv(os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResultsPerIterAttn.csv"), mode='a', header=False)
...
#folder, Comments
with open(os.path.join(args.save_path,args.type_rnn,incident_group+"/LogResultsAttn.csv"),'a', encoding='utf-7') as f:
    ...
    csv_reader.writerow([type_rnn , args.epochs,x_dim,str(args.x_length),str(args.y_length),args.learning_rate, args.hidden_layer, args.type_training, \
    						sizeTrain, args.mini_batch, args.type_actifun, round(loss_train,3), round(loss_tst,3), round(err_test,0), round(diffMaxMin,2), round(stdPreds,4), round(numPeaksTest,4),  \
