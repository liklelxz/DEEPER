...
def saveHistogramPlots(groupType, newDFTrainX, newDFValX,newDFTestX, newDFTrainY,  newDFValY, newDFTestY, folder):
    ...
    plt.savefig("{}{}_AfterTrainX".format(folder,groupType), bbox_inches='tight')
    plt.clf()
    
    plt.hist(newDFValX.reshape(-1,1))
    plt.savefig("{}{}_AfterValX".format(folder,groupType), bbox_inches='tight')
    plt.clf()

    plt.hist(newDFTestX.reshape(-1,1))
    plt.savefig("{}{}_AfterTestX".format(folder,groupType), bbox_inches='tight')
    plt.clf()

    plt.hist(newDFTrainY.reshape(-1,1))
    plt.savefig("{}{}_AfterTrainY".format(folder,groupType), bbox_inches='tight')
    plt.clf()
    
    plt.hist(newDFValY.reshape(-1,1))
    plt.savefig("{}{}_AfterValY".format(folder,groupType), bbox_inches='tight')
    plt.clf()

    plt.hist(newDFTestY.reshape(-1,1))
    plt.savefig("{}{}_AfterTestY".format(folder,groupType), bbox_inches='tight')
    plt.clf()
...
def preprocessPriods(dfGroup, listPermu, dictSplit, folder,listDateRange, x_length, y_length, sampling_method, model_type):
    ...
    train = dfGroup1['Response Time'].iloc[oneSeriesTrain.index]
    validation = dfGroup1['Response Time'].iloc[oneSeriesVal.index]
    test = dfGroup1['Response Time'].iloc[oneSeriesTest.index]

    alltrain = open(folder+groupType+'_beforePrepro.csv', "w")

    for k in range(0, len(train)):
        alltrain.write(str(train.iloc[k])+"\n")
    alltrain.close()

    allval = open(folder+groupType+'_beforePrepro.csv', "w")
    for k in range(0, len(validation)):
        allval.write(str(validation.iloc[k])+"\n")
    allval.close()

    alltest = open(folder+groupType+'_beforePrepro.csv', "w")
    for k in range(0, len(test)):
        alltest.write(str(test.iloc[k])+"\n")
    alltest.close()
    ...
    train = dfGroup['Response Time'].iloc[oneSeriesTrain.index]
    validation = dfGroup['Response Time'].iloc[oneSeriesVal.index]
    test = dfGroup['Response Time'].iloc[oneSeriesTest.index]
    scaler = None

    ########### This part is for printing the after replacement values ##############
    alltrain = open(folder+groupType+'_afterPrepro.csv', "w")
    for k in range(0, len(train)):
        alltrain.write(str(train.iloc[k])+"\n")
    alltrain.close()

    allval = open(folder+groupType+'_afterPrepro.csv', "w")
    for k in range(0, len(validation)):
        allval.write(str(validation.iloc[k])+"\n")
    allval.close()

    alltest = open(folder+groupType+'_afterPrepro.csv', "w")
    for k in range(0, len(test)):
        alltest.write(str(test.iloc[k])+"\n")
    alltest.close()
    ################################################################################
    ...
    #Not none, for the baselines (x,length)
    if model_type == None:
