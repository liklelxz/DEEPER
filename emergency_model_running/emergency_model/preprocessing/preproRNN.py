...
def sampleData(dataset,x_length,y_length):
    ...
    X = []
    Y = []
    for i in range(x_data_limit):#x_data_limit
        # for the inputs
        temp_x = []
        for j in range(x_length+y_length-1):
            temp_x.append(dataset[i+j])
        X.append(temp_x)
        # for the outputs
        temp_y = []
        for j in range(y_length):
            temp_y.append(dataset[i+offset+j])
        Y.append(temp_y)
    return X,Y
...
#75% - 25% in order
def getData(filename,x_length,y_length,percentage, input_dim=1, columns='C', typeModel=0,limitOutlier=30000):
    ...
    listCols = {'R':"Response Time",'D':"Day Number",'H':"Hour Number",'W':"Weekday"}
