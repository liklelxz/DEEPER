...
def NUMPEAK(real_test_save, pred_test_save, quantile):

    #flgPeaksReal = np.where(real_test_save>=q90)#(658, 3)
    #numPeaksReal = np.sum(flgPeaksReal,axis=1)#(2,)

    numPeaksReal = np.count_nonzero(real_test_save>=quantile,axis=1)

    #flgPeaksPred = np.where(pred_test_save>=q90)#(658, 3)
    #numPeaksPred = np.sum(flgPeaksPred,axis=1)#(2,)
    numPeaksPred = np.count_nonzero(pred_test_save>=quantile,axis=1)

    ratio  = numPeaksPred/numPeaksReal

    ratio[ratio==float('Inf')]=0

    ratioPeaks = np.mean(np.nan_to_num(ratio))
    #np.nan_to_num(
    return ratioPeaks

def NUM_NOPEAK(real_test_save, pred_test_save, quantile):
...
def getPlaceholders(org_data,comp_data):
    ...
    for i in range(len(org_data[0])):
        temp1 = []
        temp2 = []
        for j in range(len(org_data)):#300):# ################# CHANGED FOR EXCLUDING HOLIDAYS PREDICTION - RECSTATS DATA
